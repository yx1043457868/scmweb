<template>
    <div>
        <footer class="shop_cart">
            <div class="gwcInfo" :class="[integralsCount> 0 ? 'fill' : '']" @click.stop="popupVisible" ><i v-show="integralsCount>0">{{integralsCount}}</i></div>
            <button type="button" @click="commit" :disabled="disabled">兑换礼品</button>
        </footer>
...
export default {
    ...
    methods: {
        commit(){
            this.$router.push({ path: "/cofirmConvert" });
        },
        popupVisible(){
            this.$emit("popupOpen");
        },

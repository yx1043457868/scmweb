...
export default {
    ...
    methods: {
        addCart(event, food) {
            ...
            let cid = getPcid();
            // console.log(food.companyId);
            let companyId_str = food.companyId;
            if (!companyId_str) {
                MessageBox("提示", "当前货品不在预售门店范围内");
                return false;
            } else {
                if (companyId_str && companyId_str.indexOf(cid) == -1) {
                    // console.log(companyId_str.indexOf(cid));
                    MessageBox("提示", "当前货品不在预售门店范围内");
                    return false;
                }
            }
            if (food.is_enable == "N") {
                ...
                if (
                    ...
                    // }
                    food.checked = true;
                    this.$store.commit("addPreorder", foodIndex);
                    Toast({
                        message: "加入采购单成功！",
                        // iconClass: "icon icon-success",
                        duration: 950

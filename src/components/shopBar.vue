<template>
    <div>
        <footer class="shop_cart">
            <div class="gwcInfo" :class="[sampleCount> 0 ? 'fill' : '']" @click.stop="popupVisible" ><i v-show="sampleCount>0">{{sampleCount}}</i></div>
            <button type="button" @click="commit" :disabled="disabled">立即申请</button>
        </footer>
...
export default {
    ...
    methods: {
        commit(){
            this.$router.push({ path: "/cofirmSimple" });
        },
        popupVisible(){
            this.$emit("popupOpen");
        },
